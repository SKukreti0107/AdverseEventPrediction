...
class FAERSMatcher:
    ...
    def find_closest_match(self, medicine_name, threshold=0.8):
        """Find the closest matching drug in the FAERS data using fuzzy matching.
        ...
        exact_matches = self.drug_mapping[self.drug_mapping['drugname'] == normalized_name]
        if not exact_matches.empty:
            return normalized_name, 1.0
        
        # Then try fuzzy matching with fuzzywuzzy
        drug_names = list(self.drug_mapping['drugname'])
        
        # Get top 5 potential matches
        potential_matches = process.extract(normalized_name, drug_names, limit=5)
        
        for match in potential_matches:
            drug, score, *_ = match  # Unpack safely handling extra tuple elements
        ...
        # Fallback to substring matching if fuzzy matching doesn't find good matches
    ...
    def match_symptom_to_reactions(self, symptom, reactions, threshold=0.7):
        """Match a symptom to reactions in the FAERS data using fuzzy matching.
        ...
        for reaction in reactions:
            ...
            if normalized_symptom == normalized_reaction:
                return reaction, 1.0
        
        # Then try fuzzy matching with fuzzywuzzy
        # Get top 3 potential matches
        if reactions:
            potential_matches = process.extract(normalized_symptom, reactions, limit=3)
            
            for match in potential_matches:
                reaction, score, *_ = match  # Safely unpack tuple handling extra elements
                # Convert score to 0-1 range
                normalized_score = score / 100.0
    ...
    def detect_adverse_events(self, medicines, symptoms, conversation_text=""):
